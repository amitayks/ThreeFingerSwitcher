...
- **AI Command Band (optional, lives in the launcher).** An opt-in (default off) band of **on-device AI commands**. Scrub to a command, dwell to arm, lift to fire — it grabs your **selected text** (or clipboard / a screen region), runs it through an on-device **Gemma 4** model, and **streams the result into a preview canvas**. A fresh four-finger **down swipe applies** the result (replace-in-place / paste), a **horizontal swipe discards**. Commands are either **in-place edits** (rewrite/translate/summarize) or **background tasks** (add-to-calendar, save-to-project, open-a-tool-with-payload), each with an optional action-review before any side effect. The model is **multi-gigabyte and downloaded on first enable**; it runs **in-process via MLX**, so this band requires **Apple Silicon** (M-series; targets the stronger M4/M5-class chips) — there is no cloud and no low-end fallback in v1. Calendar tasks ask for Calendar permission the first time they run; nothing else new.
...
**AI commands** is a fourth opt-in (also not a gesture one; also lives in the launcher, so it needs the launcher on). Flip it on under **Settings → AI commands**. Two things to tell the user honestly: (1) enabling it starts a **one-time multi-gigabyte model download** (Gemma 4, on-device) — it shows progress in Settings and offers **Retry** if it fails (e.g. offline); the band only works once the download completes. (2) It requires **Apple Silicon** (M-series) — it runs the model in-process via MLX, so it won't function on Intel. No re-login and no new permission to enable; a **Calendar** prompt appears only the first time an add-to-calendar command actually runs. Authoring/managing commands and choosing the model is under **Settings → AI commands → Manage AI commands…**. It **relocates no gesture**.
...
- **AI Command Band:** `ai-command-band` (the synthetic AI band, command model, executor orchestration, **swipe-to-resolve** preview canvas — down swipe commits, horizontal swipe discards) — read its `design.md` for the `LLMRuntime` seam, the in-process MLX Gemma 4 runtime, the resumable download, and the swipe-to-resolve decision (D4). `harden-ai-error-handling` is the follow-up that makes every AI error surface clean + non-blocking.
...
Sources/ThreeFingerSwitcher/          ── ThreeFingerSwitcherCore library (ALL app logic; MLX-free → builds under swift build/test)
  ...
  Settings/           AppSettings (tunables + opt-ins, persisted), SettingsView, FavoritesEditorView (the "small IDE" for arranging favorites),
                      AICommandEditorView (author AI commands), ModelManagementView (download/status/evict the on-device model)
...
swift test                                    # 443 unit tests (gesture machine + launcher latching, models, grouping, layout, settings, native-gesture config, touch, AI executor/tasks/selection/canvas)
...
- **AI commands:** `enableAICommands` is a fourth opt-in (no gesture relocation, no `is…Effective` gate — like clipboard it takes effect immediately) that gates the AI band and unlocks the model download. The command list is persisted by `AICommandStore`; `aiSelectedModelID` chooses the model (resolved against `ModelRegistry`); `confirmBeforeRun` (per command) gates the action-review before a side-effecting task fires. The AI band reuses the launcher's activation/step/direction tunables; the swipe-to-resolve discard threshold reuses `clipboardPinDistance`.
...
If you (Claude) end up extending this, keep the spec in `openspec/specs/` honest, keep the 443 tests green (`swift test` for Core; `xcodebuild` for the MLX app target), and respect the landmines in **B3** — they each cost a real debugging session to learn.
