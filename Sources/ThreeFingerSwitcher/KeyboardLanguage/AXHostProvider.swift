...
final class AXHostProvider: HostProvider {
    ...
    func host(forBrowser bundleID: String) -> String? {
        ...
        // Cheap first: re-validate the address field remembered for this window (one AX read). Only a
        // miss there pays the tree walk — the one costly step — and only the WALK sits behind the
        // backoff; the cheap guards above and below (a few single attribute reads) run on every tick.
        let field: AXUIElement
        if let remembered = cachedAddressField(pid: app.processIdentifier, window: window) {
            field = remembered
        } else {
            let key = walkKey(pid: app.processIdentifier, window: window)
            guard !walkBackoff.shouldSkip(key, now: Date()) else { return nil }
            guard let found = addressField(in: window) else {
