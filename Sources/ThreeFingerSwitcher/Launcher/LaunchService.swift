...
final class LaunchService {
            ...
            p.terminationHandler = { [weak self] finished in
                Task { @MainActor in
                    ...
                    if finished.terminationStatus != 0 {
                        self?.plainOpen(url, handler: handler, completion: completion)
                    } else {
                        completion(true)
                    }
                }
    ...
    /// of them starved every other global-queue user in the process). A `Process` failure is mapped to
    /// `LaunchError` at this boundary — the notification headline is the taxonomy's clean string, the
    /// raw error text goes to the log only — and the preset tracker's completion fires exactly once.
    private func run(executable: String, args: [String], title: String, completion: @escaping FireCompletion) {
        let process = Process()
        process.executableURL = URL(fileURLWithPath: executable)
        process.arguments = args
        process.terminationHandler = { [weak self] finished in
            let failure: LaunchError? = finished.terminationStatus == 0
                ? nil : .processExited(status: finished.terminationStatus)
            Task { @MainActor in
                self?.runningProcesses.remove(finished)
        ...
        do {
            ...
            let failure = LaunchError.processStartFailed(details: String(describing: error))
            if let details = failure.copyableDetails {
                launchLog.error("\(title, privacy: .public) failed to start: \(details, privacy: .public)")
            }
            notify(title: title, body: failure.errorDescription ?? "Couldn't start.", success: false)
            completion(false)
        }
    }
