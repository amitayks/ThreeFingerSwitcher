...
/// clipboard only when provided — and nothing more.
final class WizardTourBandsTests: XCTestCase {
    ...
    func testBaseCompositionIsFlameThenDisplayAndNothingMore() {
        let bands = WizardTourBands.compose(userBands: userBands(), clipboardBand: nil)
    ...
    func testDisplayHoldsExactlyTheTwelveWindowActions() {
        let bands = WizardTourBands.compose(userBands: [], clipboardBand: nil)
        let display = bands[1]
    ...
    func testClipboardBandComesLastWhenProvided() {
        ...
        let composed = WizardTourBands.compose(userBands: userBands(), clipboardBand: clipboard)
        XCTAssertEqual(composed.count, 3)
        XCTAssertTrue(ClipboardBandBuilder.isClipboardBand(composed[2]))
    }
}
