...
## §14 — Launcher-preview realism v3 (grow-on-rehearse + real-story demo + live tunables)

A follow-on increment that takes the launcher (and band) previews from "a faithful mini that auto-loops directed strokes" to "the user's *actual* launcher that grows under the hand and obeys their settings," mirroring the onboarding playground. Scope (decided with the user): the **launcher page first**, then the **same treatment on every band page** (Clipboard / Files / AI). (Numbered §14 in tasks — the Switcher realism pass took §13.)

**Template already shipped — mirror the Switcher preview.** The Switcher increment (`HubSwitcherDemo` + `SwitcherActionMap`, in `Hub/HubSwitcherDemo.swift`) did this exact shape for the switcher: a holder owning the real model, a deterministic teaching `DemoGesture` that **changes finger count on connected contact** (`gapAfter: 0` — the switcher's 3→2 "lift one finger, keep going"), an activation-threshold→open-length map, per-frame centroid→model `navigate`, a live `setMaxScale` from the slider, and an under-preview action-map legend. The launcher work is the same pattern with a wider hand (4→2), the launcher's own tunables, a `LauncherActionMap`, **plus** the grow-on-rehearse morph (D8): the launcher morphs from preview scale to near-real size under a real swipe (the onboarding `tourPlayActive` idiom).

**D8. Grow-on-rehearse = morph into the user's ACTUAL launcher (a floating page-level overlay), NOT an in-place capped mini.**
The real launcher is large (`LauncherGridLayout.containerWidth` ≈ 740pt, `windowHeight` ≥ 280pt) — far bigger than the preview slot — so an in-place `scaleEffect` capped to the section reads as a *shrunken* launcher, not the real thing. Instead, while `playing` (a real rehearsal opened it), present the **real `LauncherView` at its native 1:1 size** as a **page-level `.overlay`** (`GrownLauncherOverlay`) over a dim backdrop, so it floats above the scrolling section content (never clipped by it) and the user **navigates the actual launcher themselves**. A `GeometryReader` shows it at **actual size whenever the Hub window fits it**, scaling down only when it can't (never clipped). The small preview stays put and recedes (`opacity 0.2`) so attention goes to the real launcher. `playing` is true **only while the user is really rehearsing**; the **idle self-playing demo stays at small preview scale** (the user's "NOT IN LARGE ONLY IN PREVIEW"). This **supersedes** the earlier "in-place `tourScale→playScale` morph capped to the column" — the user clarified the grown thing must BE the actual launcher, at real size, navigable.

**D9. Static / never-fires = the onboarding `launcherTourEnd` contract, verbatim.**
The grown preview navigates and may dwell-**arm** (the tick), but a lift only **shrinks back + `disarm()`s** — it never launches an item, opens an app, or switches a window. This is the wizard's tour contract (`launcherTourEnd`: "the tour never fires the item"). The Hub already suppresses the real recognizer while a preview is rehearsed (`hubPreviewOwnsGestures`), so the real launcher can't fire either.

**D10. The idle demo plays the real usage *story*, not abstract strokes.**
Replace the launcher's looping `GesturePose.launcherOpen()` directed-stroke sequence (open→gap→nav→gap→dismiss) with the real story: (1) **four fingers** walk horizontally exactly the **activation distance** (`launcherActivationThreshold`) → the mini launcher appears; (2) **two fingers lift while two stay down** — a finger-count change on *continuous contact*; (3) the **two fingers** continue, stepping items (horizontal) / bands (vertical); then loop. The trackpad pad's ghost and the mini launcher advance in sync, all at preview scale.
  - **The pose primitive already exists — reuse it.** The connected-stroke seam (`GesturePose.Stroke.gapAfter`; `gapAfter: 0` = no lift before the next stroke) landed with §12.1 for the switcher's 3→2 hand-off. The launcher's "4→2 without lifting" is the same primitive with a wider open stroke (`fingers: 4`, `gapAfter: 0`) followed by `fingers: 2` strokes — no new pose work, just a launcher `teachingGesture(...)` builder mirroring `HubSwitcherDemo.teachingGesture(openLength:)`. Extend the existing `gapAfter` tests to cover a 4→2 connected transition.

**D11. The preview reads SETTINGS, not fixed constants — the single source of truth.**
The onboarding tour uses hardcoded `tourStepX = 0.085` / `tourStepY = 0.13` (`FirstTouchWizardModel:93-94`) and so does *not* reflect the user's tunables; the real launcher (`GestureRecognizer:537/560/563`) reads `launcherActivationThreshold` / `launcherStepDistance` / `launcherContextStepDistance`. The Hub preview SHALL read the **settings** for activation distance, item-step, band-step, and `dwellToArmDuration`, in **both** demo and rehearse, and update **live** when a slider moves (the page already observes `AppSettings`). This makes the preview a true what-you-set-is-what-you-get surface.

**D12. Extract a shared `LauncherTourEngine(settings:)` to kill the triplication.**
The step/dwell/arm logic now exists three times — the real `LauncherOverlayController`, the wizard's `manageTourDwell` ("the controller's manageDwell, verbatim semantics"), and (newly) the Hub. Factor the odometer-with-re-baseline + dwell-arm + activation-threshold gate into one MLX-free Core engine parameterized by the live settings, consumed by the Hub launcher/band previews (and ideally retrofit the wizard tour onto it so its hardcoded constants become settings too). `swift test`-able: activation gate at the threshold, item vs band step selection by focus, dwell-arm timing, never-fires-on-lift.

**D13. Rehearse must drive the model, not just paint dots — extend the touch seam.**
Today `HubRehearseController.ingest` extracts only `contacts → liveDots` (display) while `HubDemoDriver` keeps auto-driving the model. For the user's fingers to actually drive the grown launcher, the launcher/band page needs the **full frame** (finger count + centroid) routed into its `LauncherTourEngine` when it is the active rehearse target — a per-page frame sink alongside the existing `liveDots`. Activation is the real **≥4-finger** threshold crossing (grow), continuation is **2 fingers** (navigate), `0` ends (shrink, never fire) — `handleTourTouch` re-expressed against settings. The existing ≥2-finger ownership gate and `isKeyWindow` fail-safe are unchanged.
