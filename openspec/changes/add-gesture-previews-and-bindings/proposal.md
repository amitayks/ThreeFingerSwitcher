...
- **Top-notch realism (iterative, per surface).** Each preview becomes the user's *actual* surface: the **Switcher** (§13) shows real, scalable, multi-row windows with a deterministic teaching story; the **Launcher and its band pages** (§14) idle-play the real usage story at preview scale and **grow under the hand** on a real swipe (static, never fires), with every launcher tunable reflected live. Demonstrations change finger count on *connected contact* (open → lift one/two → navigate) via the `gapAfter` seam, not abstract oscillation.
