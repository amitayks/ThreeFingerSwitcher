...
- [x] 1.1 Add `MediaKind` (`.image`/`.video`), `MediaSize` (width×height), `MediaParameters`
...
- [x] 1.2 Add the `MediaRuntime` protocol (`capabilities: Set<MediaKind>`,
...
- [x] 1.3 Add `StubMediaRuntime` (test support): scripted progress sequences — success-with-final-asset,
...
- [x] 2.1 Add `MediaToolContributor` exposing `generate_image` and `generate_video` as `ToolDescriptor`s
...
- [x] 2.2 Gate availability: contribute nothing under `mediaGenEnabled == false` /
...
- [x] 3.1 Add `MediaGenSink` invoked by the existing loop when the router selects a media tool: resolve
...
- [x] 3.2 On approval: pick the `MediaRuntime` for the kind, drive `generate(_:)`, thread `MediaProgress`
...
- [x] 3.3 Enforce the cloud-video **budget cap** before the call (a media-side mirror of the handoff
...
- [x] 3.4 Consume `ModelRegistry`/`ensureResident`; when residency math says a heavy gen must EVICT chat,
...
- [x] 4.1 Resolve `MediaRequest.seed` from the EXISTING `.screenRegion` (region picker) and
...
- [x] 4.2 A tool authored as img2img/img2video with no resolvable seed resolves
...
- [x] 5.1 Write the finished `MediaAsset.url` under a dedicated **generated-media gallery** root (local-only,
...
- [x] 5.2 Surface the asset as an ordinary `.fileEntry` in the Files band (reuse on-demand listing,
...
- [x] 6.1 Model the canvas media state: generating (live `.step` + intermediate preview) → finished
...
- [x] 6.2 Build the player overlay as a `DockPreviewOverlay`-pattern panel: **non-activating**, never
...
- [x] 6.3 Resolve by the canonical compass: **DOWN (at canvas-top) extracts** — save / paste / set-as;
...
- [x] 7.1 Feed the parked machinery the job's observable state: **thinking** badge while painting, **done**
...
- [x] 7.2 A `.dangerous` cloud-video step on a parked session calls `ParkScheduler.escalate` → needs-you +
...
- [x] 8.1 Add `enum MediaError: Error, Equatable` (`LocalizedError`): `noCapableBackend(kind)`,
...
- [x] 8.2 Map vendor/OS errors (mflux/LTXV/ComfyUI, `Process`, `NSURLError`, `FileManager`) into
...
- [x] 8.3 Write one `AuditRecord` per media step (auto/confirmed/declined/escalated/failed) through the
...
- [x] 9.1 `swift build` + `swift test` green: the seam, value types, contributor, sink, seed, gallery,
...
- [x] 9.2 `openspec validate ai-media-runtime --strict` passes; the `ai-generative-media` ADDED
