...
- **Relaxed mode becomes three-tier** (subrole allowlist + real-window discriminators):
  ...
  - *Unrecognized* subroles are listed only when the window shows identity — non-empty title OR window chrome (a close button) — and dropped as phantom otherwise, regardless of size (the AirDrop share popover births untitled chromeless host-window-sized clones that defeat any size bar; the emulator toolbar and device window are separated by title, not size). A window reporting no subrole at all stays listed (strict parity).
