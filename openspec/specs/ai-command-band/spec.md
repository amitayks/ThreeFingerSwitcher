...
Define the AI command feature surfaced in the launcher: a Codable AI-command value model persisted apart from Favorites, a Settings authoring UI, prompt-template token resolution, the synthetic AI command band built fresh per launcher open, command input acquisition with sensible fallback, in-place output routing into the captured front app, and the single opt-in that gates both the band and the on-device model.
