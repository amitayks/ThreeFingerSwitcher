...
Define the dedicated favorites editor window that browses and sources launch items by type, arranges them into context bands on a canvas, and configures per-item and per-band appearance and strategy, with edits persisting immediately to the launcher.
