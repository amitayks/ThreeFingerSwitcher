...
Define the First Touch wizard — the gesture-driven first-run experience: a transient Liquid-Glass window whose acts demo the product with the user's own hand before asking for anything, turn each permission grant into a visible in-place upgrade, gather all system-setting consent into one combined apply with one re-login, persist progress across the two restarts the flow choreographs, and complete/replay safely (including silent migration for existing installs).
