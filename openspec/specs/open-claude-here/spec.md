# open-claude-here Specification

## Purpose

Define the folder-bound terminal-launch items — the **Claude Project** item (one-tap Claude Code in a project folder) and its general **Open in Terminal** sibling (run any command in a folder) — that open the user's default terminal at a configured folder via a no-new-permission `.command` handoff, with a configurable command, a visible generated script, robust `claude` resolution, and bounded error surfacing.
