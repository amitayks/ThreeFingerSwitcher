...
Define passive raw multitouch reading via the Kyome `OpenMultitouchSupport` package, deriving finger count and per-frame velocity from raw normalized touch data and exposing a clean touch-frame stream without consuming any input event.
