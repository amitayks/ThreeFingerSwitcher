...
The system SHALL capture per-window thumbnails using ScreenCaptureKit and SHALL degrade to an app-icon placeholder when a thumbnail is unavailable or Screen Recording permission is not granted. A captured thumbnail's pixel dimensions SHALL be bounded to the size at which the card is displayed — the on-screen card size times a bounded Retina headroom factor — NOT the source window's full native-Retina resolution, so the captured bitmap stays proportional to what is shown and capture, storage, and composite cost do not scale with the source window's resolution.
...
#### Scenario: Capture is bounded to the display target
- **WHEN** a thumbnail is captured for a large (for example 4K or high-Retina) window
- **THEN** the captured bitmap's dimensions are bounded to roughly the displayed card size (times a Retina headroom factor), not the window's full native pixel resolution
